...
CameraDescriptorSet::CameraDescriptorSet(VulkanInstance* vulkanInstance,
  ...
  if (vkCreateDescriptorSetLayout(vulkanInstance->device, &setLayoutInfo,
  ...
  if (vkAllocateDescriptorSets(vulkanInstance->device, &setInfo,
                               descriptorSets.data()) != VK_SUCCESS) {
    log_ftl("Failed to allocate camera descriptor set.");
  }

  uniformBuffer = new VulkanBuffer(
      vulkanInstance, viewCount * sizeof(CameraUniform),
  ...
  for (uint32_t i = 0; i < viewCount; i++) {
    ...
    vkUpdateDescriptorSets(vulkanInstance->device, 1, &descriptorWrite, 0,
                           nullptr);
  }
}
...
CameraDescriptorSet::~CameraDescriptorSet() {
  ...
  if (setLayout != VK_NULL_HANDLE)
    vkDestroyDescriptorSetLayout(vulkanInstance->device, setLayout, nullptr);
}
...
void CameraDescriptorSet::bind(VkCommandBuffer commandBuffer,
                               uint32_t viewIndex,
