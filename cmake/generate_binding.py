...
static wasm_trap_t* componentMethodWrapper(const wasmtime_caller_t* caller, void* env, const wasm_val_t args[], wasm_val_t results[]) {
  ...
  // TODO(Turtle1331) Pop first argument
...
template <class ComponentType, BoundComponentMethod<ComponentType> method>
void linkComponentMethod(ScriptEnvironment* scripts, World* world, const char* symbol, const wasm_functype_t*& func_type) {
...
LINKER_LINK_FORMAT = "void {component_name}::linkScriptApi(ScriptEnvironment* scripts, World* world)"

LINKER_METHOD_WRAP = """\
  {{
    wasm_valtype_t* const ps[] = {{{param_types}}};
    wasm_valtype_t* const rs[] = {{{result_types}}};
    wasm_valtype_vec_t params, results;
    wasm_valtype_vec_new(&params, sizeof(ps) / sizeof(wasm_valtype_t*), ps);
    wasm_valtype_vec_new(&results, sizeof(rs) / sizeof(wasm_valtype_t*), rs);
    const wasm_functype_t* func_type = wasm_functype_new(&params, &results);

    linkComponentMethod<{component_name}, &{component_name}::{method_name}>(scripts, world, "{component_name}_{method_name}", func_type);
  }}
"""
...
class Linker(Codegen):
    ...
    def add_method(self, method_name, method):
        ...
        # TODO(Turtle1331) Infer Wasm types from C++ types
        self.methods.append(LINKER_METHOD_WRAP.format(
            component_name=f"{self.component_name}Component",
            method_name=method_name,
            param_types=", ".join(f"wasm_valtype_new_{valtype}()"
                for valtype in method.get("params_wasm", [])),
            result_types=", ".join(f"wasm_valtype_new_{valtype}()"
                for valtype in method.get("return_wasm", []))))

    def finish(self):
        # Implement Component::linkScriptApi()
        component_name = f"{self.component_name}Component"
        self.out.extend([
            LINKER_LINK_FORMAT.format(component_name=component_name),
            " {"])

        self.out.extend(self.methods)
